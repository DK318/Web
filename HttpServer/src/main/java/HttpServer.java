...
class HttpServer {
    ...
    private void makeResponse(Response response, File file) throws IOException {
        byte [] body = readFile(file);
        response.setStatusCode(200);
        response.setBody(body);
        response.setHeader("Content-Length", Integer.toString(body.length));
        response.setHeader("ETag", Integer.toString(Arrays.hashCode(body)));
    }
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String url = URLDecoder.decode(request.getUri(), StandardCharsets.UTF_8.toString());
        File file = new File(root, url);
        if (file.isFile()) {
            String previousETag = request.getHeader("If-None-Match");
            if (previousETag != null && previousETag.equals(Integer.toString(Arrays.hashCode(readFile(file))))) {
                response.setStatusCode(304);
                response.setHeader("ETag", previousETag);
            } else {
                makeResponse(response, file);
            }
            response.setHeader("Content-Type", getContentType(file));
        } else if (file.isDirectory()) {
            File indexFile = new File(file.getPath() + "/index.html");
            if (indexFile.exists()) {
                makeResponse(response, indexFile);
