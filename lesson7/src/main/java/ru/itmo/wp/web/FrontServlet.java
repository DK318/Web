...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method actionMethod = null, beforeMethod = null, afterMethod = null;
        for (Class<?> clazz = pageClass; (actionMethod == null ||
                beforeMethod == null ||
                afterMethod == null) && clazz != null; clazz = clazz.getSuperclass()) {
            ...
            for (Method cur : methods) {
                if (cur.getName().equals(route.getAction()) && actionMethod == null) {
                    actionMethod = cur;
                }
            }
        }

        if (actionMethod == null || beforeMethod == null || afterMethod == null) {
            throw new NotFoundException();
        }

        String acceptRequestHeader = request.getHeader("Accept");
        boolean json = actionMethod.getAnnotation(Json.class) != null
            ...
            for (Class<?> type : types) {
                if (type == HttpServletRequest.class) {
                    ...
                    throw new ServletException("Illegal signature for route's action method " +
                            "[method=" + actionMethod + "type=" + type + "]");
                }
            }
            actionMethod.invoke(page, args.toArray());
