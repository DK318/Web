...
public class UserService {
    ...
    public void validateAdminUpdate(long sourceUserId, long targetUserId, boolean admin) throws ValidationException {
        User sourceUser = findById(sourceUserId);
        User targetUser = findById(targetUserId);

        if (!sourceUser.isAdmin()) {
            throw new ValidationException("You should be admin");
        }

        if (targetUser.isAdmin() == admin) {
            throw new ValidationException("Target user is already admin/not admin");
        }
    }

    public void updateAdmin(long userId, boolean admin) {
        userRepository.updateAdmin(userId, admin);
    }
